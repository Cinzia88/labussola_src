...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.vocePHotel.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.voce-p-hotels.update", [$vocePHotel->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="info_aggiuntive">{{ trans('cruds.vocePHotel.fields.info_aggiuntive') }}</label>
                <textarea class="form-control {{ $errors->has('info_aggiuntive') ? 'is-invalid' : '' }}" name="info_aggiuntive" id="info_aggiuntive">{{ old('info_aggiuntive', $vocePHotel->info_aggiuntive) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('hotel') ? 'is-invalid' : '' }}" name="hotel_id" id="hotel_id" required>
                    @foreach($hotels as $id => $entry)
                        <option value="{{ $id }}" {{ (old('hotel_id') ? old('hotel_id') : $vocePHotel->hotel->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('hotel'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('preventivo') ? 'is-invalid' : '' }}" name="preventivo_id" id="preventivo_id">
                    @foreach($preventivos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('preventivo_id') ? old('preventivo_id') : $vocePHotel->preventivo->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
