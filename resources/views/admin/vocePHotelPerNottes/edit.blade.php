...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.vocePHotelPerNotte.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.voce-p-hotel-per-nottes.update", [$vocePHotelPerNotte->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tipologia_stanza') ? 'is-invalid' : '' }}" name="tipologia_stanza" id="tipologia_stanza" required>
                    ...
                    @foreach(App\Models\VocePHotelPerNotte::TIPOLOGIA_STANZA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tipologia_stanza', $vocePHotelPerNotte->tipologia_stanza) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('numero_stanze') ? 'is-invalid' : '' }}" type="number" name="numero_stanze" id="numero_stanze" value="{{ old('numero_stanze', $vocePHotelPerNotte->numero_stanze) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('costo_a_notte') ? 'is-invalid' : '' }}" type="number" name="costo_a_notte" id="costo_a_notte" value="{{ old('costo_a_notte', $vocePHotelPerNotte->costo_a_notte) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('voce_hotel') ? 'is-invalid' : '' }}" name="voce_hotel_id" id="voce_hotel_id" required>
                    @foreach($voce_hotels as $id => $entry)
                        <option value="{{ $id }}" {{ (old('voce_hotel_id') ? old('voce_hotel_id') : $vocePHotelPerNotte->voce_hotel->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('scorpora') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="scorpora" id="scorpora" value="1" {{ $vocePHotelPerNotte->scorpora || old('scorpora', 0) === 1 ? 'checked' : '' }}>
