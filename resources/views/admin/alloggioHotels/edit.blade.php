...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.alloggioHotel.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.alloggio-hotels.update", [$alloggioHotel->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nome">{{ trans('cruds.alloggioHotel.fields.nome') }}</label>
                <input class="form-control {{ $errors->has('nome') ? 'is-invalid' : '' }}" type="text" name="nome" id="nome" value="{{ old('nome', $alloggioHotel->nome) }}" required>
                @if($errors->has('nome'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('indirizzo') ? 'is-invalid' : '' }}" type="text" name="indirizzo" id="indirizzo" value="{{ old('indirizzo', $alloggioHotel->indirizzo) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('descrizione') ? 'is-invalid' : '' }}" name="descrizione" id="descrizione">{{ old('descrizione', $alloggioHotel->descrizione) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('stelle') ? 'is-invalid' : '' }}" name="stelle" id="stelle" required>
                    ...
                    @foreach(App\Models\AlloggioHotel::STELLE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('stelle', $alloggioHotel->stelle) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('fornitore') ? 'is-invalid' : '' }}" name="fornitore_id" id="fornitore_id">
                    @foreach($fornitores as $id => $entry)
                        <option value="{{ $id }}" {{ (old('fornitore_id') ? old('fornitore_id') : $alloggioHotel->fornitore->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
