...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.trasporto.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.trasportos.update", [$trasporto->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nome">{{ trans('cruds.trasporto.fields.nome') }}</label>
                <input class="form-control {{ $errors->has('nome') ? 'is-invalid' : '' }}" type="text" name="nome" id="nome" value="{{ old('nome', $trasporto->nome) }}" required>
                @if($errors->has('nome'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('descrizione') ? 'is-invalid' : '' }}" name="descrizione" id="descrizione">{{ old('descrizione', $trasporto->descrizione) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('fornitore') ? 'is-invalid' : '' }}" name="fornitore_id" id="fornitore_id">
                    @foreach($fornitores as $id => $entry)
                        <option value="{{ $id }}" {{ (old('fornitore_id') ? old('fornitore_id') : $trasporto->fornitore->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
