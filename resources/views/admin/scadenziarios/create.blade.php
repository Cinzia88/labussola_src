...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.scadenziario.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.scadenziarios.store') }}" enctype="multipart/form-data">
                @csrf
                    ...
                    <select class="form-control select2 {{ $errors->has('preventivo') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($preventivos as $preventivo)
                            <option value="{{ $preventivo->id }}"
                                {{ old('preventivo_id') == $preventivo->id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('colore_eticchetta') ? 'is-invalid' : '' }}"
                        name="colore_eticchetta" id="colore_eticchetta" required>
                        <option value disabled {{ old('colore_eticchetta', null) === null ? 'selected' : '' }}>
                            {{ trans('global.pleaseSelect') }}</option>
                        @foreach (App\Models\Scadenziario::COLORE_ETICCHETTA_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('colore_eticchetta', '') === (string) $key ? 'selected' : '' }}>{{ $label }}
                            </option>
                        @endforeach
                    </select>
...
@section('scripts')
    <script>
        const selectList = document.getElementById('preventivo_id');
        const queryString = window.location.search;
        const urlParams = new URLSearchParams(queryString);
        const price = urlParams.get('preventivo'); // change "price" to anything you want.
